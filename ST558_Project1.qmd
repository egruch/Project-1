...
if ("SEX" %in% names(API_data)) {
 ...
 for (var in char_var)
  test_tibble <- API_data |> 
   # The only way I could find to convert a "string" to a variable name 
   mutate("SEX_new" = get(paste0(var, "_dict"))[get(var)]) |>
...
```{r}
test_tib <- call_API(2022, c("PWGTP", "AGEP"), "SEX", "state", "09")
test_tib
```

...
```{r}
call_API <- function(my_url, year=2022, num_vars = c("PWGTP", "MAR"), char_var = "SEX", geo_level = "state", geo_subset = "09") {
  if (!is.numeric(year) | year < 2010 | year > 2022) {
  ...
  API_parsed <- jsonlite::fromJSON(rawToChar(my_url$content))
  
  colnames(API_parsed) <- API_parsed[1,]
  API_parsed <- API_parsed[-1,]
  API_tibble <- as_tibble(API_parsed)
  #class(API_tibble) <- c("census", class(API_tibble))
  
if ("SEX" %in% names(API_tibble)) {
  API_tibble <- API_tibble |>
    mutate(SEX = ifelse(SEX == "1", "Male", 
                         "Female")) |>
    mutate(SEX = factor(SEX))
} else if ("FER" %in% names(API_tibble)) {
  API_tibble <- API_tibble |>

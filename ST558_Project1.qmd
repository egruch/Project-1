...
NARRATIVE: ADDRESS WHAT FUNCTION DOES GENERALLY. THEN IN COMMENTS ADDRESS MINOR DETAILS.

HAVE EXAMPLES OF OUTPUT AND DESCRIBE IT.
...
call_API <- function(year=2022, num_vars = c("PWGTP", "AGEP"), cat_vars = c("SEX"), geo_level = "state", geo_subset = "09") {
  if (!is.numeric(year) | year < 2010 | year > 2022 | year == 2020) {
    ...
    stop("Entered an invalid numeric variable")
                             ...
                             "SCHL", "SEX"))) {
    stop("Entered an invalid character variable")
    ...
    stop("HISPEED and JWTRNS only have data for 2019 and 2021")
...
summary(API_call_multi_year, c("all"), c("all"))
```
...
## PUMS Census API Data North Carolina Exploration
...
We can see that the number of people with no internet or no high speed internet decreased from 2019 to 2021, and the number with high speed internet increased. This supports our general conjecture. The mean GRPIP went down, which means that either people were making more income by 2021, or rent prices had gone down.

...
"Must have a narrative throughout the document. A story or thread that flows throughout the document."
...
"Our overall goal is to write functions that will manipulate and process data sets that come from a census API."

"We want the user to choose certain variables to return and choose certain conditions on what is returned (specific subsets)."

## Data

Maybe write anything about the API and data itself.

"We are going to query the Public Use Microdata Sample (PUMS) Census API. This API gives pseudo person-level data."

PWGTP represents the number of observations (people) for a particular row.

# Data Processing

"Goal is to write a function to query this API and return a nicely formatted tibble."
...
} else if (!(geo_subset %in%
             c(names(state_dict),names(division_dict), 
               names(region_dict))
             )) {
  "BAD7"
...
call_API <- function(year=2022, num_vars = c("PWGTP", "AGEP"), char_var = c("SEX"), geo_level = "state", geo_subset = "09") {
  if (!is.numeric(year) | year < 2010 | year > 2022 | year == 2020) {
    ...
    stop("Entered  an invaild character variable") 
    
  } else if (("HISPEED" %in% char_var | "JWTRNS" %in% char_var) & 
             (year == 2019 | year == 2021)) {
    stop("No data for HISPEED or JWTRNS in 2019 or 2021")
    ...
    stop("Entered an invalid geography subset")
  ...
  # Go through the rest of the numerical variables and replace values
    ...
    if (var == "AGEP") {
      ...
      API_data <- API_data |>
        mutate(!!var := ifelse(get(var) == "3", NA, get(var)))
    } else if (var == "GRPIP") {
      API_data <- API_data |>
...
#multi_year_API <- function(years, num_vars = c("PWGTP", "AGEP"), char_var = c("SEX"), geo_level = "state", geo_subset = "09") {
  
#  API_list <- list()
  
# for (i in 1:length(years) ) {
    
#    list_tibble_name <- paste0("API_", years[i])
#    API_list[[list_tibble_name]] <- call_API(years[i], 
#                                         num_vars, char_var,
#                                         geo_level, geo_subset)
    
    #get(paste0("API_", years[i])) <- call_API(years[i], 
                                        # num_vars, char_var,
                                        # geo_level, geo_subset)
  #API_list <- append(API_list, get(paste0("API_", years[i])))  
#  }
  
#  API_comb <- do.call(rbind, API_list)
#  return(API_comb)
}
```

```{r}
test_tibble <- call_API_mult_years(c(2022),
